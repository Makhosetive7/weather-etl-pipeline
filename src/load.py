...
class WeatherLoader:
    ...
    def load_city(self, city_data: Dict) -> Optional[int]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                result = conn.execute(insert_query, city_data)
    ...
    def load_weather_condition(self, condition_data: Dict) -> Optional[int]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                result = conn.execute(insert_query, condition_data)

                conn.commit()

                condition_id = result.fetchone()[0]
                logger.info(
                    f"Inserted new condition: {condition_data['description']} (ID: {condition_id})"
                )
                return condition_id
    ...
    def load_measurement(self, measurement_data: Dict, city_id: int, condition_id: int) -> bool:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                data = {"city_id": city_id, "condition_id": condition_id, **measurement_data}
    ...
    def load_weather_record(self, transformed_data: Dict) -> bool:
        ...
        try:
            ...
            # Load measurement
            success = self.load_measurement(transformed_data["measurement"], city_id, condition_id)

            if success:
                city_name = transformed_data["city"]["city_name"]
                logger.info(f"Successfully loaded complete weather record for {city_name}")

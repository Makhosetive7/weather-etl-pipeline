...
class weather_extractor:
        def __init__(self):
                ...
                if not self.api_key:
                        raise ValueError("API key not configured properly. Please check OPENWEATHER API KE IN THE .env file")
                ...
                def fetch_weather_by_city(self, city_name: str, country_code: str = None) -> Optional[Dict]:
                        try:
                            ...
                            if e.response.status_code == 401:
                                logger.error("Invalid API KEY. Please check you Weather API key in the .env file")
                ...
                def test_api_connection(self) -> bool:
                    try:
                        ...
                        if data:
                            logger.info("API connection succesfully")
                            return True
                        else: 
                        ...
                        logger.error(f"API connection testing failded: {e}")
                        return False
